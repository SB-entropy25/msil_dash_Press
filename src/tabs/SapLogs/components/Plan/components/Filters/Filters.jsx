...
const Filters = ({ filters = {}, onChange = () => {}, domain = {} }) => {
  ...
  const handleEndTime = (val) => {
    ...
    //   const params = {
    //     open: true,
    //     message: "Scheduled finish must not be greater than current time",
    //     type: "ALERT",
    //   };
    //   dispatch(setApplicationAlert(params));
    // }
      ...
      const params = {
        open: true,
        message: "Scheduled finish must be greater than Scheduled start",
        type: "ALERT",
      };
      dispatch(setApplicationAlert(params));
    }
    ...
    >
      ...
      >
        <MultiSelect
          ...
          options={domain?.order_status}
          selectedOptions={filters?.order_status}
          handleToggle={(props) => handleToggle(props, "order_status")}
        />
        <MultiSelect
          name="order_type"
          label="Order type"
          handleClearAll={handleMultiClearAll}
          options={domain?.order_type}
          selectedOptions={filters?.order_type}
          handleToggle={(props) => handleToggle(props, "order_type")}
        />
        <Box>
          <MultiSelectFilter
            disabled={false}
