...
const MultiCheckboxSelect = ({
  ...
  const menuItems = options?.flatMap((item, index) => {
    const selected = areAllGroupOptionsSelected(item.options);
    const items = [
      <MenuItem
        className={`prodigi-multiselect-group ${
          selected ? "prodigi-multi-selected" : ""
        }`}
        key={item.label}
        value={item}
        selected={areAllGroupOptionsSelected(item.options)}
        onClick={handleGroupToggle(item.options)}
      >
        <ListItemIcon>
          <Checkbox
            size="large"
            ...
            indeterminate={
              !areAllGroupOptionsSelected(item.options) &&
              item.options?.some((option) => selectedOptions?.includes(option))
            }
          />
        </ListItemIcon>
    ...
    if (item.label !== "All") {
      items.push(
        item.options?.map((item) => (
          <ListItemButton
            className="prodigi-multiselect-item"
            key={item}
            value={item}
